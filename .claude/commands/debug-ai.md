...
- **AI: Rust** — PyO3 製 Rust 拡張（`src/Othello.AI/Python/othello_ai_rust.pyd`）が使用されている
...
- 探索深さが難易度に応じて正しく設定されているか (Beginner:1, Easy:2, Medium:5, Hard:10, Expert:12)
- αβカットオフ・トランスポジションテーブルが正しく機能しているか

**Evaluator** (`src/Othello.AI/CSharp/Evaluator.cs`)
- 評価値: 位置重み（コーナー +100, X-square -50 等）+ Mobility/Stability/Frontier を空きマス数に応じたフェーズ（序盤/中盤/終盤）で組み合わせる（詳細: `src/Othello.AI/CSharp/README.md`）
