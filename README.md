...
- 難易度 5 段階（ビギナー / イージー / ノーマル / ハード / エキスパート）
- 有効手のハイライト表示（WPF・WinUI3 版：黄色、コンソール版：`*`）
- 有効手がない場合は自動パス（画面にメッセージ通知）・1手戻す（Undo）機能
- 最初の一手が置かれるまで難易度・担当色を変更可能
- ヒント機能: C# AI（探索深さ 2）による推奨手のハイライト表示（人間ターン中にオン/オフ切替可）
- 制限時間モード: 手番ごとの持ち時間を設定し、時間切れ時は有効手の先頭を自動着手
- CPU vs CPU 対戦モード: 黒・白それぞれに難易度を設定し AI 同士の対局を観戦（速度調整・一時停止可）

### 棋譜・統計
- 棋譜保存/再生: 対局を棋譜ファイルとして保存し、専用ウィンドウで1手ずつ・先頭/末尾へ手順を再生
- 棋力統計: 難易度別の勝敗数・引き分け数・連勝記録・最高石数差を記録
- スコア推移グラフ: 対局中の黒白の石数推移をグラフで表示
...
- 難易度別探索深さ: ビギナー=1、イージー=2、ノーマル=5、ハード=10、エキスパート=12
- **Hard/Expert 難易度では全バックエンドで反復深化（時間制限 Hard=8秒、Expert=15秒）を使用**
...
| カバレッジ計測 + レポート生成 | `Ctrl+Shift+P` → `Tasks: Run Task` → `test: Report`（`test: Coverage` に依存する2段階タスクで、計測→レポート生成まで自動実行） |
...
│   │   ├── Game/              # GameEngine
│   │   ├── Kifu/              # KifuRecord, KifuMove, KifuPlayer, KifuFinalScore, KifuSerializer
...
│   │   ├── StatsViewModel.cs  # 棋力統計表示用 ViewModel
│   │   └── ScorePoint.cs      # スコア推移グラフ用の1点分のデータ
│   │
...
│   │
│   ├── Othello.Tests/         # xUnit テスト（.NET 10、クロスプラットフォーム）
│   └── Othello.Tests.WPF/     # xUnit テスト（WPF 依存。Converter 等 System.Windows 型を使うテストのみ）
│
...
盤面の空きマス数によってフェーズ（序盤/中盤/終盤）を判定し、重みを動的に切り替える（`evaluator.py` / `Evaluator.cs` / `lib.rs` 共通仕様、`test_parity.py` が一致を担保）。
...
- **位置重み**（全フェーズ共通）: コーナー（+100）を最優先で取りにいく。X-square（-50、コーナー斜め隣）はリスクが高いため避ける
- **Mobility**（着手可能数の差）: 選択肢が多いほど有利。序盤×20、中盤×10、終盤×10
- **Stability（安定石）**（中盤のみ、×25）: コーナーから連鎖的に確定するひっくり返せない石の差
- **Frontier（フロンティア）**（中盤のみ、×5）: 空きマスに隣接する石（不安定の代理指標）の差。少ない方が有利
- **石数差**（終盤のみ、×10）: 空きマスが 20 未満になったら石数差を主成分にする

| フェーズ | 空きマス数 | 評価式 |
|---|---|---|
| 序盤 | 44 超 | 位置重み + Mobility×20 |
| 中盤 | 20〜44 | 位置重み + Mobility×10 + Stability×25 + Frontier差×5 |
| 終盤 | 20 未満 | 石数差×10 + 位置重み + Mobility×10 |
...
Technopro.Othello.Core.Kifu     # KifuRecord, KifuMove, KifuPlayer, KifuSerializer
Technopro.Othello.Core.Settings # OthelloSettings, OthelloSettingsManager
Technopro.Othello.Core.Stats    # GameStats, DifficultyStats, StatsRepository
