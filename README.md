...
| バッジ | バックエンド | 条件 |
|--------|------------|------|
| **AI: Rust** | PyO3 製 Rust 拡張 | Rust ビルド済み & Python あり |
| **AI: Python** | 純 Python 実装 | Python あり（Rust 未ビルド） |
...
dotnet build Othello.sln -p:Platform=x64
...
Hard 難易度では `time_ms=8000` を付加し、反復深化（Iterative Deepening）を使用します。
