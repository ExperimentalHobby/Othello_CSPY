...
| `depth` | `int` | アルファベータ探索の最大深さ（1 以上必須）。難易度に対応（Easy:2, Normal:5, Hard:10） |
| `time_ms` | `int \| null` | 反復深化探索の時間制限（ミリ秒）。Hard=`8000`、Easy/Normal=`null`（固定深さ探索） |
...
- **固定深さ**（Easy / Normal）: `get_best_move(board, player, depth)` を使用
- **反復深化**（Hard）: `get_best_move_timed(board, player, depth, time_ms)` を使用。時間切れ前の最深深さの結果を採用する
...
> AI 本体の **Rust 化（高速化）・反復深化（全バックエンド）は実施済み**です。以下はさらなる改善案で、いずれも Rust 実装（`Othello.AI/Rust`）側で取り組むのが効果的です。
