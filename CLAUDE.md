...
- **Issue をクローズする際はチェックボックスの完了状況を見直すこと**: タスクリスト形式の Issue をクローズする前に、各項目が実際に完了しているかをコード・CI 実行結果で確認し、完了している項目はチェックを入れてからクローズする。Issue 本文に書かれた文言をそのまま信用しない。実装方法が Issue の記述と異なるなど完了と断定できない場合は、独断でチェックせずユーザーに確認する
